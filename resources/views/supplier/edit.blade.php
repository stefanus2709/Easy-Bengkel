<div>
    <form action="/supplier/update" method="POST" id="editForm">
        @csrf
        @method('PATCH')
        <input type="hidden" name="supplier_id" id="supplier_id">
        <div>
            <label for="inputSupplierName" class="form-label">Supplier Name</label>
            <input type="text" class="form-control mb-3" name="name" id="name" placeholder="Input Supplier Name" @error('name') is invalid @enderror>
        </div>
        <div>
