...
<div class="px-4 py-4 main-content">
    ...
    <form action="/quotation/update/{{$quotation->id}}" method="POST">
        ...
        @method('PATCH')
        <div class="mb-3">
            <label for="inputPurchaseInDate" class="form-label">Customer Name</label>
        ...
        <div class="mb-3">
            <label for="inputPurchaseInDate" class="form-label">Quotation Date</label>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="total_price" name="total_price" placeholder="Input Total Price"
                value="{{$quotation->total_price}}" disabled>
            @error('total_price')
            <span class="text-danger">{{$message}}</span>
            @enderror
        </div>
        ...
        <div class="row g-3 align-items-center mb-3">
            ...
            @if (!blank($products))
                ...
                <select id="select-product" class="selectpicker form-control" data-live-search="true" multiple
                    data-max-options="1" name="product_id" onchange="getProductId('selling_price', this.selling_price.value)">
                    @foreach ($products as $product)
                    <option value="{{$product->id}}" data-rc="{{$product->selling_price}}">{{$product->name}}</option>
                    @endforeach
                </select>
            ...
            <div class="col-sm">
                <label for="sellingPrice" class="form-label">Selling Price</label>
                    ...
                    <input type="format_po" class="form-control" id="selling_price" name="selling_price" placeholder="Input Selling Price" disabled>
        ...
        <table class="table" id="datatable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @if (!$quotation->finalized)
                    <td style="width: 10%;">
                        <a href="/po_in/{{$quotation->id}}/details/edit/{{$detail->id}}" class="btn btn-info fs-16px"><i
...
<div class="modal" id="finalizePurchaseInModal" tabindex="-1" aria-labelledby="finalizePurchaseInModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <p class="modal-title fs-22px" id="finalizePurchaseInModalLabel">Finalize Quotation</p>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div>
